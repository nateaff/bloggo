...
```
You can also glance at the fields in a table without loading the table into memory using `dplyr`. With `dplyr` we call a specific table with the `tbl` function and then manipulate the object `dplyr` returns and treat that object (for a limted set of functions) as if it were a dataframe. 

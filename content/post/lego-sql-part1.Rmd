...
The main goal of the and a planned follow-up tutorial is to introduce several packages useful for interfacing with relational databases from R. In this tutorial, we will walk through setting up a Postgres database with the `RPostgreSQL` package. The `RPostgreSQL` package is one of several packages that use the standardized interface provided by the `DBI` package. Aside from setting up the initial connection to the database, the code here could be used with another database for which the `DBI` interface has been implemented. For example, the `RMySQL` and `RSQLite` are available for the SQLite and MySQL databases. 
...
* RStudio has a nice overview of database related packages [here](https://db.rstudio.com/). 

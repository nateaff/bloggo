...
When I was introduced to fractal(or fractional) dimension it was in the abstract setting of sets. Sometimes fractal dimension is also lumped into the category of nonlinear features that are associated with dynamical systems. 
There is a more direct relation, though, to more familiar statistical properties of random processes. On top of that, changes in fractal dimension are fairly easy to identify in the graph of a function.

Roughly speaking, fractal dimension is a local property of a function (more precisely, its graph); Fractal dimension measures a relation of points that are close to each other in the time series. In contrast, the Hurst parameter is a measure of a global property, a measure of a relation of points that are distant from each other. 
...
Both of the Hurst parameter and fractal dimension properties are related to the behavior of the autocorrelation function of a random process. 

The [autocorrelation function](https://en.wikipedia.org/wiki/Autocorrelation) is the correlation, or similarity, of a function values with itself at various lags $h$. To simplify things, we assume we have a random process with mean 0 and variance 1. Then the expression for the autocorrelation function is

$$
  c(h)  = E[X(t)X(t + h)],  h \in \mathbb{R^n}.
$$
...
Some random processes, [Brownian Motion](), for instance, have a single parameter that determines both the fractal dimension and the Hurst parameter of the function. 
The Cauchy process, on the other hand, has two parameters that separately control the fractal dimension($D$) and the long-range dependence or Hurst parameter($H$). The Cauchy process can be described by its correlation function:
$$
  c(h) = \left(1+ |h|^{\alpha}\right)^{-\beta/\alpha},  h \in \mathbb{R}^n
$$
...
The plot below is a set of Cauchy processes corresponding to a grid of paramters $\pi \times \pi$, $\pi = (0.3, 0,95, 1.6)$. The plots are labeled with the parameter grid translated into the fractal dimension $D$ and the Hurst parameter $H$. Fractal dimension is constant on the **columns** and the Hurst parameter is constant on the **rows**.
...
plot_transects <- function(k){
  plot(xs[[k]][,10], col = viridis::viridis(10)[3], 
                    type = 'l',
                    lwd = 1.6,  
                    axes = FALSE)
  add_text(k)
...
Given the relation of fractal dimension to the autocorrelation function(ACF), we should see the difference in the empirical ACF of each of these time series.
...
For these time series, the plot below shows the values of a fractal dimension estimator against the Hurst parameter of the functions. The plot looks like the parameter grid because changes in the Hurst parameter aren't affecting the fractal dimension estimator. (The plot matches the functions above when rotated 90 degrees counter-clockwise.)
...
plot(coeffs[,2], lapply(transects, get_fd),  
            pch = 15,
            cex = 1.7,
...
I started the post asserting that fractal dimension could be a useful feature for the classification or clustering of time series. Naturally, this depends on whether your time series have the kind of variation in local behavior that is caught by the fractal dimension estimator. 
...
t1 <- replicate(reps, arima.sim(n = 500, list(ar = c(0.7, -0.5), ma = c(-5, 0,5))))
t2 <- replicate(reps, arima.sim(n = 500, list(ar = c(0.9, -0.2), ma = c(-5, 0,5))))  
...
df1 <- apply(t1, 2, get_fd) %>% data.frame
df2 <- apply(t2, 2, get_fd) %>% data.frame
df3 <- apply(t3, 2, get_fd) %>% data.frame

fd_df <- rbind(df1, df2, df3)
fd_df$id <- as.factor(c(rep("Sim 1", reps), 
                        rep("Sim 2", reps), 
                        rep("Sim 3", reps))
                        )

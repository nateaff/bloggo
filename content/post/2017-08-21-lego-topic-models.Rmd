...
The code for generating the analysis and plots, along with a little math background on the evaluation methods, is in [this repo](github.com/nateaff/legolda) and this [Kaggle notebook](kaggle.com/nateaff/finding-lego-color-themes-with-topic-models) as the same analysis but run a smaller sample of sets.

One motivation for doing this analysis was to try some methods from the handy [Text Mining in R](tidytextmining.com) book by Julia Silge and  David Robinson and some of my code follows their examples. In particular, in the TF-IDF section and the analysis of the distribution of documents over the topics. 
